...
class ConsumeDriver(threading.Thread):
        ...
        while True:
            ...
            if len(msg) > 0:
                ...
                if(topic=="test_config"):
                    # print(type(message))
                    self.configs[message["testID"]]={"testType":message["testType"],"testDelay":message["testDelay"],"requests":message["requests"]}
                elif(topic=="trigger"):
    ...
    def run_test(self,message):
        if(message["testID"] in self.configs.keys()):
            ...
            x = self.configs[message["testID"]]["requests"]
            self.driver_side_latency = []
            for i in range(x):
                ...
                self.driver_side_latency.append((exit-entry)*1000)
                data = {
                    "node_id": self.nodeID,
                    "test_id": message["testID"],
                    "report_id": str(uuid.uuid4()),
                    "metrics": {
                        "mean_latency": statistics.mean(self.driver_side_latency),
                        "median_latency": statistics.median(self.driver_side_latency),
                        "min_latency": min(self.driver_side_latency),
                        "max_latency": max(self.driver_side_latency),
